...
async function loadBoxScore(gameId) {
  try {
    const { data } = await client.get('https://www.espn.com/college-football/boxscore?gameId=' + gameId, {
      timeout: 30000
    });
    const boxscoreHtml = getHtml(data, 'article.boxscore-tabs');
    return boxscoreHtml;
  } catch (err) {
    console.log('ERROR RETRIEVING BOXSCORE FOR ' + gameId + '\n' + err);
  }
}
...
async function getPassingTotals(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO FIND PASSING TOTALS FOR ' + gameId);
  }
  return stats;
}
...
async function getRushingTotals(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO LOAD RUSHING TOTALS FOR ' + gameId);
  }
  return stats;
...
async function getReceivingTotals(gameId) {
  try {
    ...
    console.log('FAILED LOAD RECEIVING TOTALS FOR ' + gameId);
  }
...
async function getDefenseTotals(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO FIND DEFENSE TOTALS FOR ' + gameId);
  }
...
async function getFumbles(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO LOAD FUMBLES FOR ' + gameId);
  }
  return stats;
}
...
async function getInterceptions(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO LOAD INTERCEPTIONS FOR ' + gameId);
  }
...
async function getKickReturns(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO LOAD KICK RETURNS FOR ' + gameId);
  }
  return stats;
}
...
async function getPuntReturns(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO LOAD PUNT RETURNS FOR ' + gameId);
  }
  return stats;
}
...
async function getKicking(gameId) {
  ...
  try {
    ...
    console.log('FAILED TO LOAD KICKING FOR ' + gameId);
  }
  return stats;
}
...
async function getPunting(gameId) {
  try {
    ...
    console.log('FAILED TO LOAD PUNTING FOR ' + gameId);
  }
  return stats;
}
